...
detect_skill_domain() {
    ...
    if [[ "$file" =~ /drain/ ]]; then
        detected_skills="drain"
    elif [[ "$file" =~ /router/ ]]; then
        detected_skills="router"
    elif [[ "$file" =~ /tests/ ]] || [[ "$file" =~ /conftest ]]; then
        detected_skills="tests"
    elif [[ "$file" =~ /workqueue/ ]]; then
        detected_skills="workqueue"
    elif [[ "$file" =~ /conductor/ ]] || [[ "$file" =~ /orchestrator/ ]] || [[ "$file" =~ /taskformats/ ]] || [[ "$file" =~ /addons/ ]] || [[ "$file" =~ /workqueue/ ]] || [[ "$file" =~ /router/ ]]; then
        detected_skills="worktrail"
    fi
