...
error for the whole brief's re-check, per the Requirement: Deterministic Predicate Re-Check
Precedes Evidence Surfacing.

#### Scenario: Shell metacharacters are never interpreted
- **WHEN** a captured command's argument vector contains a token with shell metacharacters
  (for example `;`, `|`, or `$(...)`)
- **THEN** the token is passed to the executed process literally, with no shell interpretation

#### Scenario: A string command is rejected without being shell-split
- **WHEN** a `drift-findings` entry's `predicate-cmd` is a single string rather than a list
- **THEN** the entry is treated as malformed and the whole brief's re-check errors, without the
  string ever being shell-split or executed

#### Scenario: A missing executable errors the re-check
- **WHEN** a captured command names an executable that does not exist on `PATH`
- **THEN** the resulting failure to launch the process is treated as an error for the whole
  brief's re-check

#### Scenario: A hanging command times out
- **WHEN** a captured command exceeds its per-command wall-clock timeout
- **THEN** the command is treated as an error for the whole brief's re-check, not as either
  still-holding or resolved

#### Scenario: An over-cap brief errors before executing any command
- **WHEN** a brief's `drift-findings` list carries more entries than the bounded per-brief
  command cap
- **THEN** the whole brief's re-check is treated as an error, and no captured command for that
  brief is executed
