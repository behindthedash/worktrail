...
contains an explicit re-home directive — a re-home, move, retarget, reassign, or re-scope verb
(hyphen optional in "re-home" and "re-scope", matched case-insensitively) followed
...
#### Scenario: Re-scope directive naming an unknown repo is ignored
- **WHEN** the answer is "Re-scope it to the nonesuch repo" and no `nonesuch` checkout exists
- **THEN** the brief and decision are unchanged and no unresolvable entry is reported
