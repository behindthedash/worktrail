...
When a user runs `worktrail-go <brief-id>` and the brief is an intake brief, the system SHALL
run the intake-triage evaluation for that single brief under exactly the same rules as the
unattended pre-pass — repo inference and write-back, the mechanical premise check, the
null-repo rule, bounded keep with escalation, and the same validation and apply semantics —
and SHALL apply the resulting verdict without asking the user to confirm it, since every
verdict path other than a genuine product decision is deterministic and a product decision is
already filed as `needs-decision`. The session SHALL report the action-log entry the apply
step returns, including landing any resulting pull request through the shared PR-landing
pipeline in the same invocation; when that outcome is a code defect or blocking review
threads, the session SHALL continue the CI watch loop's repair procedure against the
reported branch and run record rather than stopping at "PR opened". When the applied verdict
is `work-directly`, the session SHALL continue in the same invocation into the normal
claim + classify + dispatch flow for the now-execution brief instead of stopping. For every
other verdict the brief SHALL NOT be claimed into `picked/` by this path.

#### Scenario: User names an intake brief
- **WHEN** `worktrail-go 20260826-143940-consolidated-...` is invoked and that brief has no
  `seeded-from:`
- **THEN** the session evaluates the brief against its repo's active changes, applies the
  verdict without a confirmation prompt, and reports the action-log entry
...
- **WHEN** the applied verdict is `fold-into-change` and the apply step returns a PR URL
  with a landed outcome
- **THEN** the session reports the PR URL and the completion state the pipeline recorded,
  and stops

#### Scenario: Confirmed propose reports a code defect
- **WHEN** the applied verdict is `propose-change` and the apply step returns a PR URL with
  a code-defect outcome
- **THEN** the session does not stop at the report; it repairs the defect in the reported
  worktree and re-invokes the landing pipeline against the same run record until a terminal
  outcome is reached
