# triage-evaluator-live-smoke Specification

## Purpose
Prove that a real triage evaluator emits a `work-directly` verdict with a verbatim `refuted_span`, by recording one live run with a hand-run harness and replaying it offline through `parse_verdicts()` and `apply_verdicts()`.
## Requirements
