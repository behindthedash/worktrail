...
- [x] 2.1 Add optional `effort: Optional[str]` to `policy._validate_agent_entry()`
- [x] 2.2 Thread `effort` through `resolve_routing()`'s returned `fallback`/`roles`/
      `tiers` entry shape
- [x] 2.3 Unit tests: entry with `effort` validates and resolves; entry without
...
- [x] 4.1 Document (README/skill doc) how to set `routing.roles: {review: {agent_cli:
      claude, agent_model: opus}}` in `~/.go/routing.yaml`
- [x] 4.2 Document the 3-tier complexity fallback
      (`trivial`/`standard`/`hard` → `gpt-5.6-luna`/`gpt-5.6-terra`/`gpt-5.6-sol`, or
      whatever values 1.1 confirms) as a `routing.tiers` example
- [x] 4.3 [e2e] Regression test: full suite + golden check (`orchestrate check`) green with
