...
- [ ] 3.1 Add a `main(argv=None)` CLI to `check_brief_predicate.py` (`--repo`, `--brief`,
...
- [ ] 3.2 Add a CLI-level test (or extend `test_check_brief_predicate.py`) exercising the
...
- [ ] 5.1 In `skills/worktrail-go/references/brief-staleness-check.md`, add a new section
      ...
      step.
- [ ] 5.2 Document the `"still-true"` branch: append the run-record evidence line (task 4.1's
      ...
      branch's timing).
- [ ] 5.3 Document the `"resolved"` branch: call `worktrail-work-queue done` with the closure
...
- [ ] 5.4 Document that `attempted: false` (outcome `"no-predicate"`, `"unrecognized"`, or
      ...
      invocation.
- [ ] 5.5 Update the module's cost/bounds note (if any new subprocess cost is material — reading
