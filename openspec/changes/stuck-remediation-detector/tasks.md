...
- [x] 3.2 In `tests/drain/test_drain.py`, add coverage that `drain()`'s `summary["stuck_remediations"]` is an empty list when no identity recurs, that an identity recurring across `stuck_threshold` sequential `drain()` calls sharing a history path is flagged with the expected streak, and that a `--dry-run` (or `repos_root=None`) invocation never writes the history file.
