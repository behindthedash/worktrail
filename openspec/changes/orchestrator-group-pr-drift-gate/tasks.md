...
- [x] 2.2 Branch on it after `policy = load_policy(repo)` and after the `--labels-only`
...
- [x] 3.3 Add `_run_drift_gate(iw: Path, name: str) -> tuple[bool, str]` modeled on
...
- [x] 3.4 Call it in `integrate_one` immediately after
...
- [x] 4.1 Regression test for requirement:
...
- [x] 4.3 Test that a drift failure short-circuits **before** the smoke command runs
  (assert the smoke command was never invoked when a `smoke_cmd` is configured).
- [x] 4.4 Happy-path regression: a clean group with a passing gate still runs its smoke
  command, pushes, and opens its PR with unchanged labels.
- [x] 4.5 Fail-closed test: an unresolvable gate script (or a timeout) quarantines the
  group rather than allowing it through.
- [x] 4.6 Use the existing `PRE_PR_GATE_SCRIPT` env override to substitute a stub gate in
...
- [x] 6.4 [cleanup] Run `openspec validate orchestrator-group-pr-drift-gate --strict` and
  confirm it passes.
