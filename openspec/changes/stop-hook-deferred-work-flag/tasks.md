...
- [x] 1.1 Create `src/worktrail/router/check_deferred_work_handoff.py`: load one or more
...
- [x] 1.2 Add a `DEFERRAL_PHRASES` module-level constant and a case-insensitive
...
- [x] 1.4 Implement the CLI `main()`: repeatable `--run-record PATH`, `--json`, printing
...
- [x] 1.5 Register `worktrail-check-deferred-work-handoff =
...
- [x] 2.2 After the existing sentinel/substantive-work gate, locate
...
- [x] 2.3 When the subprocess reports at least one flagged entry, append a second,
...
- [x] 3.1 `tests/router/test_check_deferred_work_handoff.py`: a run record's
      ...
      Signal Source)
- [x] 3.2 Same file: phrase-matching entries become candidates; non-matching entries are
      never flagged regardless of handoff coverage (Requirement: Deferral-Phrase Matching)
- [x] 3.3 Same file: a candidate whose extracted probes match an existing `queue/` or
...
- [x] 3.4 `hooks/test_suggest_next_step.py`: a transcript containing a run-record path
...
- [x] 3.5 Same file: a transcript with a run-record path literal whose `deferred_work` has
      ...
      And Non-Interfering)
- [x] 3.6 Same file: a transcript with no run-record path literal, a missing
