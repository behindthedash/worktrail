...
- [ ] 1.4 Add a helper that, given `next_n` and the full epic text, returns the set of candidate
      gating feature numbers from both patterns (blanket matches filtered to `< next_n`).

## 2. Dashboard: gate resolution and new stage

- [ ] 2.1 Add a helper that resolves one candidate gating feature number `M` to closed/open per
      ...
      "complete"}`).
- [ ] 2.2 In `detect_epic_stage()`, after computing `features`/`cited` and confirming
...
- [ ] 2.3 Confirm `scan_epics()` needs no changes beyond returning whatever
      `detect_epic_stage()` now returns (it already forwards the dict as-is).

## 3. Seeder: skip-and-report the new stage

- [ ] 3.1 In `find_epic_gaps()`, add an `epic-sequencing-gated` branch alongside the existing
...
- [ ] 3.2 In `seed_backlog()`, log one line per sequencing-gated finding (epic id, blocked
...
- [ ] 3.3 Add a `sequencing_gated_epics` key to `seed_backlog()`'s returned summary dict,
      parallel to `unparseable_epics` (list of `{"repo": ..., "id": ...}`).

## 4. Tests: dashboard gate detection

- [ ] 4.1 In `tests/router/test_dashboard.py`, add a case: epic with pairwise gate prose
...
- [ ] 4.2 Add a case: same epic, but Feature 1's future spec id is cited by a spec/change folder
      whose `dashboard.scan()` stage is `ready-to-implement` (the 0/16-tasks incident shape) ->
      still `epic-sequencing-gated`.
- [ ] 4.3 Add a case: same epic, Feature 1's citing spec/change folder's stage is `done` ->
      `epic-gap` (seedable), unchanged from today's behavior.
- [ ] 4.4 Add a case: Feature 1's citing folder exists only under `openspec/changes/archive/
...
- [ ] 4.5 Add a case: blanket gate prose ("Feature 1 gates the rest") with an open Feature 1 ->
...
- [ ] 4.6 Add a case: gate prose names a feature other than `next_n` (e.g. "Feature 3 depends on
...
- [ ] 5.1 In `tests/workqueue/test_seed_backlog.py`, add a case: an epic in
      `epic-sequencing-gated` stage produces no queued brief and no `seed_key` collision with a
      later sweep.
- [ ] 5.2 Add a case: `seed_backlog()`'s returned summary includes the epic under
      `sequencing_gated_epics`.
- [ ] 5.3 Add a case: the sweep's log output includes a line naming the epic, the blocked feature
      number, and the gate's resolved state.
- [ ] 5.4 Add a regression case: an epic already covered by an existing `find_epic_gaps`/
