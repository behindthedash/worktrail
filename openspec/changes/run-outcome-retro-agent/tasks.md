...
- [x] 1.1 In `src/worktrail/learning/paths.py` and `src/worktrail/learning/digest.py`, add `learning_dir(repo)`, `retro_memory_path(repo)`, `RETRO_AGENT_NAME`, and the pure `build_outcome_digest(journal)` per design D1-D2. Also create the empty `src/worktrail/learning/__init__.py` and `tests/learning/__init__.py`.
...
- [x] 2.1 In `src/worktrail/router/policy.py`, add the flat `agent_learning` boolean to the policy defaults with value `False`, per design D8.
...
- [x] 3.1 In `src/worktrail/learning/retro.py` and `src/worktrail/learning/retro_agent.md`, implement:
