...
- [ ] 1.1 In `src/worktrail/router/cluster_detect.py`, change
...
- [ ] 2.2 Wire the verification function into `_filter_reportable()` (or
...
- [ ] 2.3 Apply a 10-second timeout to the verification call; treat
...
- [ ] 3.1 Add fixtures to `tests/router/test_cluster_detect.py` covering:
      ...
      verification call.
- [ ] 3.2 Add a regression fixture using the real PR #93 pair (both
...
- [ ] 3.3 Add fail-open fixtures: mocked timeout, mocked non-zero exit,
      mocked empty output, and no-agent-CLI-configured, each asserting the
      pair is not surfaced and no exception propagates.
- [ ] 3.4 Update `test_one_null_repo_blocks_repo_scoped_signals` and
...
- [ ] 4.1 [e2e] Run `PYTHONPATH=src pytest -q` and confirm green.
