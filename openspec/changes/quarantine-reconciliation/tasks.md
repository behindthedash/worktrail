...
- [ ] 1.1 In `src/worktrail/router/quarantine_selfcheck.py`, add
...
- [ ] 2.1 Add `_files_on_base(repo: Path, base: str, files: List[str]) -> bool`:
      for each path in `files`, run `git -C repo ls-tree <base> -- <path>` (or
      equivalent) and return `True` only if every path resolves. `base`
      defaults to the repo's current checked-out branch
      (`git -C repo rev-parse --abbrev-ref HEAD`) when not passed explicitly.

## 3. Merged-PR reconciliation signal

- [ ] 3.1 Add `_merged_pr_matching(repo: Path, files: List[str]) -> Optional[str]`:
...
- [ ] 4.1 Add `reconcile_finding(repo: Path, finding: Dict[str, Any]) ->
...
- [ ] 4.2 In `check_repo()`, after building the raw findings list exactly as
...
- [ ] 4.3 Update `sweep()` to include a repo's result whenever `findings` is
      non-empty (unchanged from today — `reconciled`-only repos are not
      "flagged").

## 5. Tests

- [ ] 5.1 Add `tests/router/test_quarantine_selfcheck.py` cases for
...
- [ ] 5.2 Add cases for `_files_on_base()`: all files present on base → `True`;
      one file missing → `False`; explicit `base` argument used over current
      branch.
- [ ] 5.3 Add cases for `_merged_pr_matching()` (mock `subprocess.run`): a
      ...
      (never raises).
- [ ] 5.4 Add cases for `reconcile_finding()`/`check_repo()`: a QUARANTINED
...
- [ ] 5.5 [e2e] Run `PYTHONPATH=src pytest -q` and `PYTHONPATH=src python3 -m
