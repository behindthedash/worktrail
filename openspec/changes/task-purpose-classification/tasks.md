...
- [ ] 1.1 Add optional `purpose: Optional[str]` to `taskformats/devkit/schema.py`'s
      `FIELD_SCHEMA`, alongside `complexity`/`domain`
- [ ] 1.2 Add `purpose: str` to `taskformats/base.py`'s `TaskDict`
- [ ] 1.3 Unit tests: a devkit task with `purpose` set loads and validates; a
...
- [ ] 2.1 In `conductor/compile.py`, resolve the target repo's
      `routing.purpose_tiers` (via `policy.py`, see task 4) before building
      `PROMPT`
- [ ] 2.2 When `routing.purpose_tiers` is non-empty: extend `PROMPT` to
...
- [ ] 2.3 Extend `TaskPlan` with a `purpose: str = ""` field; `_validate()`
...
- [ ] 2.4 In `conductor/runplan.py`'s `apply_to_tasks()`, add `"purpose"` to
      the existing `for field in ("complexity", "review")` merge-only-if-
      absent loop
- [ ] 2.5 Unit tests: repo with configured `routing.purpose_tiers` gets a
...
- [ ] 3.1 Add `routing.purpose_tiers: Dict[str, str]` validation to
      `policy.py` (plain string-to-string map, no agent-entry shape)
- [ ] 3.2 Thread `purpose_tiers` through `resolve_routing()`'s returned dict,
      alongside `fallback`/`roles`/`tiers`
- [ ] 3.3 Unit tests: a configured `routing.purpose_tiers` table resolves and
...
- [ ] 4.1 Add a `purpose_tier_map: Optional[Dict[str, str]] = None` parameter
      to `agent_for()`
- [ ] 4.2 Implement tier-name resolution: `purpose_tier_map.get(task.get
      ("purpose"))` if it resolves, else `task.get("complexity")` — for
      `implement`/`fix`/`cleanup` roles only; `JUDGMENT_ROLES` untouched
- [ ] 4.3 Implement the two-step `tier_map` lookup: try
      ...
      matches
- [ ] 4.4 Unit tests: purpose-derived tier takes precedence over complexity;
...
- [ ] 5.1 Thread `resolve_routing()`'s `purpose_tiers` result through to every
...
- [ ] 5.2 [e2e] Regression test: full suite + golden check (`orchestrate
...
- [ ] 6.1 Document `routing.purpose_tiers` in the same README/skill doc
...
- [ ] 6.2 Update `openspec/changes/model-tier-routing/tasks.md`'s §5 (5.1,
