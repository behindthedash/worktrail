...
`_is_spec_doc`/`_rank`/tie-check logic (see `dashboard.py:150-211`). Both
...
`dashboard_selfcheck.py` imports `_is_spec_doc` and `_rank` from
`dashboard.py` rather than duplicating the candidate-classification rules.
...
shared logic to import). Rejected — `find_spec_file()`'s ambiguity rule is
exactly what this check flags; a second independent implementation of the
same classification would drift the moment one side changes (this is the
same class of bug — undetected silent divergence — the check itself exists
...
- [Risk] `dashboard.py`'s private `_is_spec_doc`/`_rank` helpers are not a
  published API; a future rename inside `dashboard.py` would break this
  import silently until tests catch it. → Mitigation: `test_dashboard_selfcheck.py`
  imports and exercises these helpers directly (not just via subprocess),
  so a rename fails the test suite immediately rather than at runtime.
