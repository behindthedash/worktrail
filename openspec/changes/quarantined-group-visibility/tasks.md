...
- [x] 1.1 Create `src/worktrail/router/quarantine_selfcheck.py` implementing
      ...
      posture).
- [x] 1.2 In the same file, add `sweep(repos_root: Path) -> List[Dict[str, Any]]`
...
- [x] 1.3 Add the `worktrail-quarantine-selfcheck` console-script entry point to
...
- [x] 1.4 Add `tests/router/test_quarantine_selfcheck.py` covering: no
...
- [x] 2.1 In `src/worktrail/router/dashboard.py`, import
...
- [x] 2.2 In `scan_repos()`, call `_quarantine_check_repo(repo)["findings"]` per
...
- [x] 2.3 In `render_dashboard()`, build a `quarantine_flags` list from each repo
...
- [x] 2.4 Extend `tests/router/test_dashboard.py`: `scan_repos()` returns
...
- [x] 3.2 [e2e] Run `worktrail-quarantine-selfcheck --repo <this-checkout>`
