...
- [x] 5.1 Extend `tests/onboarding/test_repo_init.py`: fresh repo run writes both the workflow and a ruleset with the required check. (Requirement: Scaffold a portable openspec-validate workflow — Scenario: Fresh repo)
- [x] 5.2 Already-onboarded repo (openspec initialized, workflow absent, ruleset files already exist): run writes the workflow and patches the existing ruleset files to add the check, without altering their other rules. (Requirement: Wire the new check into required_status_checks, scoped to this job only — Scenario: New workflow written this run, ruleset file already exists)
