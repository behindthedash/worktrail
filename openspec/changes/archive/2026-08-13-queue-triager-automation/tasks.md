...
- [x] 1.1 Create `src/worktrail/workqueue/queue_triage.py`; add `group_queue_by_repo()` that
...
- [x] 1.2 Add `is_recently_triaged(path, within_days) -> bool` that scans the brief body for
      the most recent `## Triage <ISO date>` section and compares against `within_days`.
- [x] 1.3 Add `inventory(within_days) -> (groups, skipped)` composing 1.1 + 1.2: briefs failing
      the dedup check are excluded from `groups` and returned in `skipped` for report visibility.

## 2. Evaluator prompt and spawn

- [x] 2.1 Add the module-level evaluator prompt template (mirrors `drain.py`'s `PROMPT`
...
- [x] 2.2 Add `evaluate_group(repo, briefs, *, agent, model, cwd) -> List[dict]` that builds
...
- [x] 2.3 Add `parse_verdicts(raw_text, expected_brief_ids) -> List[Verdict]` implementing the
...
- [x] 2.4 Add the archived-repo short-circuit: before 2.2's spawn, run
...
- [x] 3.1 Add `write_verdict_file(verdicts, out_dir) -> Path` (JSON) and
...
- [x] 3.2 Add the `evaluate` CLI subcommand: `worktrail-queue-triage evaluate
...
- [x] 4.1 Add `resolve_duplicate_targets(verdicts) -> List[Verdict]` implementing the
...
- [x] 4.2 Add `apply_verdicts(verdicts, *, confirm) -> List[dict]` (action log): for each
...
- [x] 4.3 Add the `apply` CLI subcommand: `worktrail-queue-triage apply --verdict-file PATH
      [--confirm] [--json]`. Loads the verdict file, runs 4.1 → 4.2, prints the action log.

## 5. Packaging

- [x] 5.1 Add `worktrail-queue-triage = "worktrail.workqueue.queue_triage:main"` to
      `pyproject.toml`'s `[project.scripts]`, alphabetically placed among the other
      `worktrail-*` entries.
- [x] 5.2 Add the recommended-cadence note (monthly / pre-drain-weekly, not nightly — ~1M
      tokens per full run) to the module docstring, matching `drain.py`'s docstring
      precedent.

## 6. Tests

- [x] 6.1 `tests/workqueue/test_queue_triage.py`: grouping (repo grouping, null-repo
      collapsing), dedup-marker detection (recent vs. stale `## Triage` section).
- [x] 6.2 Verdict parsing: well-formed verdict, malformed output falls back to `keep` with raw
      text retained, missing-evidence falls back to `keep`.
- [x] 6.3 Archived-repo short-circuit: confirmed archival synthesizes group-wide
...
- [x] 6.4 Apply step: `--confirm` false is a pure dry run (assert no filesystem mutation);
...
- [x] 6.5 Dangling `duplicate-of` resolution: target verdicted non-`keep` in the same batch
      downgrades the referencing verdict to a no-op with a logged warning.
- [x] 6.6 Report/verdict-file output: counts in the Markdown report match the JSON file
      exactly for a synthetic multi-group run.

## 7. Verification

- [x] 7.1 [cleanup] `PYTHONPATH=src pytest -q` green.
- [x] 7.2 [cleanup] `PYTHONPATH=src python3 -m worktrail.orchestrator.orchestrate check` green.
- [x] 7.3 [cleanup] `pytest tests/test_plugin_surface.py` green (no skill/plugin surface
