...
- [x] 2.1 Change `_task_files_are_shipped(repo, files, tracked)` to
...
- [x] 2.2 Update the three existing calls inside `dashboard.py` (`_pending_impl_stale`,
...
- [x] 3.1 In `verify()`, compute `since_ts = dashboard._dir_creation_timestamp(str(repo),
...
- [x] 4.1 In `tests/router/test_dashboard.py`'s `StaleBookkeeping._spec_dir()`, `git add` +
...
- [x] 4.2 In `tests/router/test_dashboard.py`'s `OpenSpecStaleBookkeeping._change()`, `git add` +
...
- [x] 4.3 [e2e] Run the full `StaleBookkeeping` and `OpenSpecStaleBookkeeping` test classes and
...
- [x] 5.1 Add a `_commit_at`-style helper (explicit `GIT_AUTHOR_DATE`/`GIT_COMMITTER_DATE`) to
...
- [x] 5.2 Add `test_preexisting_unchanged_file_is_not_stale`: commit a file at `t0`, create+commit
...
- [x] 5.3 Add `test_new_file_since_creation_is_stale`: create+commit the change directory at
      ...
      `stale_task_ids`.
- [x] 5.4 Add `test_preexisting_file_modified_after_creation_is_stale`: commit a declared file at
...
- [x] 6.1 [e2e] Run `PYTHONPATH=src pytest -q` and confirm the full suite is green, including
