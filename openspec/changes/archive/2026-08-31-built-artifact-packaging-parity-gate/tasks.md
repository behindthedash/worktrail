...
- [x] 1.3 Add sdist metadata extraction: read `PKG-INFO` from the
      built/given sdist via `tarfile`, parsing its `Version` header the
      same way as the wheel's `METADATA`. (Requirement: Built-artifact
      metadata extraction)
- [x] 1.4 Raise a clear, distinguishable error (not an unhandled exception)
...
- [x] 2.1 Import and reuse `checkout_console_scripts(repo)` and
      `checkout_version(repo)` from `scripts/check_packaging_metadata.py`
      for the checkout's declared values — do not re-parse
      `pyproject.toml`.
- [x] 2.2 Compare the wheel's and sdist's extracted version against the
...
- [x] 2.3 Compare the wheel's extracted `console_scripts` name set against
...
- [x] 2.4 Add a `main()`/CLI entry point mirroring
      ...
      pass.
- [x] 2.5 Ensure the temporary build directory (when the script builds
      artifacts itself) is always removed, including on failure paths.

## 3. Tests

- [x] 3.1 Add `tests/test_built_artifact_packaging.py`: a matching-checkout
      fixture (using this repo's own checkout, or a minimal synthetic
      `pyproject.toml` fixture) builds cleanly and the check passes.
- [x] 3.2 Test version drift: a synthetic checkout/artifact pair with
      differing versions fails with a message naming both values.
- [x] 3.3 Test console-script drift: missing-script and extra-script cases
      each fail with a message naming the differing script name(s).
- [x] 3.4 Test build failure handling: a build subprocess failure (e.g. a
      malformed `pyproject.toml`) produces a clear, non-crashing error.
- [x] 3.5 Test missing-metadata-file handling: an artifact archive lacking
      its expected metadata file produces a clear, non-crashing error
      naming the missing file.
- [x] 3.6 Test hermeticity: running the check does not modify the checkout's
...
- [x] 3.7 Test the `--wheel`/`--sdist` pre-built-artifact path: given
      existing artifact paths, the check reads them without invoking a
      build (e.g. assert the build subprocess is not invoked).

## 4. CI wiring

- [x] 4.1 Extend `.github/workflows/ci.yml`'s `lint-test-build` job: after
...
- [x] 4.2 Confirm no new job or required-status-check name is introduced —
...
- [x] 5.1 [e2e] Run `PYTHONPATH=src pytest -q` and confirm the new tests
      pass alongside the full suite.
- [x] 5.2 [e2e] Run `PYTHONPATH=src python3 -m
      worktrail.orchestrator.orchestrate check` (golden regression) and
      confirm it remains green.
- [x] 5.3 [e2e] Run `python3 scripts/check_built_artifact_packaging.py
      --repo .` locally against this checkout and confirm it reports
      success.
- [x] 5.4 [e2e] Confirm `tests/test_plugin_surface.py` still passes (no
