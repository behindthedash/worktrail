...
- [x] 1.1 In `src/worktrail/learning/notes.py`, add `load_learned_notes(repo) -> str | None` per design D3. It reads `worktrail.learning.paths.retro_memory_path(repo)` (from `run-outcome-retro-agent`) and returns the `## Notes for workers` bullets up to the next `## ` heading.
...
- [x] 2.1 In `src/worktrail/orchestrator/dispatch.py`, add `learned_notes: str | None = None` to `WorkerPromptCtx`. When it is non-empty, render the learned-notes heading from the spec plus the notes text immediately before `"Hard rules:"` in `build_worker_prompt`. Render nothing otherwise.
...
- [x] 3.1 In `src/worktrail/orchestrator/live.py`, add a lazy `LiveSpawn.learned_notes` property that mirrors `pre_commit_cmd`.
