...
- [x] 1.1 Create `src/worktrail/workqueue/backfill_focus_style.py`: `build_preview(queue_base)`
...
- [x] 1.2 Implement the span-splice rewrite: given a proposal, re-locate the `focus:` value
...
- [x] 1.3 Implement `execute_apply(preview, queue_base, confirm)` mirroring
      ...
      check fails.
- [x] 1.4 Implement `main()`/CLI with `preview` and `execute` subcommands, `--queue-dir`
      override, and `execute`'s `--preview`/stdin + `--confirm`/`--decline` flags, matching
      `backfill_created_quoting.py`'s argument shape exactly.
- [x] 1.5 Register the console script entry point in `pyproject.toml`:
...
- [x] 2.1 `tests/workqueue/test_backfill_focus_style.py` — `build_preview`: proposes a
...
- [x] 2.2 `tests/workqueue/test_backfill_focus_style.py` — span-splice/execute: for each
...
- [x] 2.3 `tests/workqueue/test_backfill_focus_style.py` — safety contract: `decline` writes
      ...
      content.
- [x] 2.4 `tests/workqueue/test_backfill_focus_style.py` — CLI: `execute` reads the preview JSON
...
- [x] 3.1 [e2e] Run `worktrail-backfill-focus-style preview` against the real
      ...
      proceeding.
- [x] 3.2 [e2e] Run `worktrail-backfill-focus-style execute --confirm` against the real corpus
...
- [x] 3.3 [e2e] Re-run `worktrail-backfill-focus-style preview` against the real corpus and
