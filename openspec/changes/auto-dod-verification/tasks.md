...
- [x] 2.2 Wire into `check_task_file`: when `dod-checks` is absent or empty,
...
- [x] 2.3 Confirm explicit `dod-checks` (non-empty) still short-circuits
      derivation entirely — no merging of explicit and derived checks.

## 3. Backlog audit mode

- [x] 3.1 Implement `audit_all_specs(repo) -> list[str]`: walks
...
- [x] 3.2 Add `--all` flag to `check_dod_verification.py`'s `main()`: when
...
- [x] 3.3 [cleanup] Confirm `pre_pr_gate.py` is unchanged apart from transitively
      picking up derived checks through its existing
      `check_dod_failures(repo, changed_paths(...))` call.

## 4. Tests

- [x] 4.1 `run_check` unit tests: `file_tracked` pass/fail (missing path,
...
- [x] 4.2 `derive_dod_checks` unit tests: `files:` present → file_tracked +
...
- [x] 4.3 `check_task_file` unit tests: explicit `dod-checks` present skips
...
- [x] 4.4 `TestClarificationIntegrityGate`-style real-git-repo integration
      ...
      entry → returns `0`.
- [x] 4.5 `audit_all_specs` / `--all` test: a repo fixture with a `completed`
...
- [x] 5.1 Update `check_dod_verification.py`'s module docstring to describe
      the derivation fallback and `--all` audit mode alongside the existing
      explicit-`dod-checks` description.
- [x] 5.2 Verify `pre_pr_gate.py`'s module docstring (which cites
...
- [x] 6.1 [cleanup] `PYTHONPATH=src pytest -q` passes.
- [x] 6.2 [cleanup] `PYTHONPATH=src python3 -m worktrail.orchestrator.orchestrate check`
      passes.
- [x] 6.3 [cleanup] `tests/test_plugin_surface.py` passes unchanged (no new console
      script, no new skill).
