...
- [x] 2.1 Add `build_rulesets_drift_guard_workflow(branches: List[str]) -> str` that renders the
...
- [x] 2.2 Add a final always-run step in the generated job that prints a one-line notice when the
      credential guard was false (App not configured), so a skipped run's logs explain why.

## 3. Wire scaffolding into `cmd_propose`

- [x] 3.1 Extend `detect_state()` with `rulesets_drift_guard_exists` /
      `rulesets_sync_script_exists` checks (mirroring `automerge_workflow_exists`).
- [x] 3.2 In `cmd_propose`, after the rulesets JSON loop (which already computes `branches`),
...
- [x] 4.1 Add a helper (e.g. `app_credentials_configured(gh_repo) -> bool`) that checks for
...
- [x] 4.2 In `cmd_apply`, after the existing rulesets apply loop, call the helper and append a
...
- [x] 5.1 In `tests/onboarding/test_repo_init.py`, add propose tests: fresh repo writes both the
      workflow and vendored script files; re-running propose skips an already-present
      `rulesets_drift_guard.yml`.
- [x] 5.2 Add a test asserting `build_rulesets_drift_guard_workflow(["dev", "prd"])` and
      ...
      call.
- [x] 5.3 Add a test asserting the generated workflow's credential-guard `if:` conditions are
      present on the token-mint and rulesets-check/apply steps.
- [x] 5.4 Add `cmd_apply` tests (mocked `gh` calls) covering: reminder printed when
