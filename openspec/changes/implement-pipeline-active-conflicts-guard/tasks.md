...
- [ ] 1.1 In `skills/worktrail-go/references/subagent-prompts.md`, add a new
...
- [ ] 1.2 Rewrite `#sibling-worktree-check` in the same file to open with a
...
- [ ] 2.1 In `skills/worktrail-sdd-workflow/references/pipeline-details.md`,
  ...
  orchestrator.
- [ ] 2.2 In the same section, add one sentence noting that no advisory glob
...
- [ ] 3.1 [e2e] Run `PYTHONPATH=src pytest -q tests/test_plugin_surface.py`
  ...
  `#implement-pipeline`.
- [ ] 3.2 [e2e] Run the full repo gate: `PYTHONPATH=src pytest -q &&
  PYTHONPATH=src python3 -m worktrail.orchestrator.orchestrate check`.
- [ ] 3.3 [e2e] Read both edited files
