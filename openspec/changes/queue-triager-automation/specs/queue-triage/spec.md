...
verdict of exactly one of `keep`, `stale-close`, `needs-update`, or `duplicate-of`, and SHALL
require non-empty `evidence` text for every verdict. A verdict that is missing, malformed, or
missing required evidence SHALL be recorded as `keep` with the evaluator's raw output retained
as evidence text, never silently dropped from the output.
...
`stale-close`, `needs-update`, or `duplicate-of` action, and SHALL take no queue-mutating
action for any `keep` verdict. `apply` SHALL NOT expose any flag or code path that closes or
edits a brief without both an existing verdict file entry and the `--confirm` flag.
...
- **THEN** every planned action (claim+done for stale-close/duplicate-of, in-place edit for
  needs-update) is printed but no brief in `queue/` or `picked/` is modified
