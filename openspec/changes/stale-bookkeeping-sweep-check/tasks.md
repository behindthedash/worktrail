...
- [x] 3.1 Import `file_stale_bookkeeping_brief` and `check_repo_stale_bookkeeping` in
      `spec_sync_sweep.py`, alongside the existing checkbox imports.
- [x] 3.2 Extend `_empty_record()` with `"stale_bookkeeping_drifted": []`,
...
- [x] 3.3 In `run_sweep()`'s per-repo loop, after the existing checkbox-drift check, add a third
...
- [x] 3.4 Extend `main()`'s human-readable summary `print(...)` call to append a third clause
...
- [x] 4.2 Add `tests/router/test_spec_sync_sweep_stale_bookkeeping_brief.py`: filing with
      ...
      every finding.
- [x] 4.3 Update `tests/router/test_spec_sync_sweep.py`'s `run_sweep()` coverage to add
...
- [x] 4.4 Update `tests/router/test_spec_sync_sweep_e2e.py` if it asserts on the full record
      shape or summary line, so it covers the new `stale_bookkeeping_*` fields end-to-end.
- [x] 4.5 [cleanup] Run `PYTHONPATH=src pytest -q` and `PYTHONPATH=src python3 -m
...
- [x] 5.1 Update `spec_sync_sweep.py`'s module docstring to describe the stale-bookkeeping check
...
- [x] 5.2 Add a deployment note (in the same docstring, near the existing cron example) stating
