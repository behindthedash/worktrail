...
- [x] 1.4 In `drain()` (`drain.py:1390`), replace the `acquire_lock(config.lock_file)` call and
      ...
      (`drain.py:1564-1565`).
- [x] 1.5 Add `--max-workers` to the CLI parser (`drain.py:main`, near the existing
      `--consecutive-failures`/`--iteration-timeout-minutes` block), resolved per D3/section 6
      below rather than a bare `default=1`.
- [x] 1.6 Tests in `tests/drain/test_drain.py`: two sequential `acquire_lock_slot` calls with
...
- [x] 2.2 In `src/worktrail/drain/drain.py`, wrap `record_capacity_gate()`'s
...
- [x] 2.3 Test in `tests/orchestrator/test_agent_capacity.py`: two threads (or two sequential
...
- [x] 3.1 In `src/worktrail/drain/drain.py`, extend `newest_run_record()`
...
- [x] 3.2 In `drain()`'s main loop (`drain.py:1438` onward), when
      ...
      (no filter).
- [x] 3.3 Tests in `tests/drain/test_drain.py`: `newest_run_record` with `repo_filter` set
...
- [x] 4.1 In `src/worktrail/drain/drain.py`, add `worker_scratch_dir(slot: int, home: Optional[Path]
      = None) -> Path` returning `(home or worktrail_home()) / "drain-workers" /
      f"worker-{slot}"`.
- [x] 4.2 Add `cwd: Optional[Path] = None` to `run_one_shot()`'s signature (`drain.py:1376-1387`)
      and pass it through to `subprocess.run(..., cwd=str(cwd) if cwd else None, ...)`.
- [x] 4.3 In `drain()`, after acquiring a slot (task 1.4), call `worker_scratch_dir(slot).mkdir
...
- [x] 4.4 Test in `tests/drain/test_drain.py`: `drain()` with `max_workers=2` and an injected
...
- [x] 5.1 In `drain()` (`drain.py:1420-1435` pre-loop, `drain.py:1554-1563` post-loop), guard
...
- [x] 5.2 Test in `tests/drain/test_drain.py`: `drain()` invoked with a slot other than 0 (inject
...
- [x] 6.2 In `drain.py:main()`, resolve `max_workers` as: `args.max_workers` if passed, else
...
- [x] 6.3 Update `docstring`/`--help` text for `--max-workers` in `drain.py:main()` to state the
      CLI > config > built-in precedence, matching the existing `--fallback-agent` help text
      style.
- [x] 6.4 Tests in `tests/shared/test_operator_config.py`: `drain_config()` returns
...
- [x] 7.1 [cleanup] Update `drain.py`'s module docstring (the `Usage:` block,
      `drain.py:79-88`) to include `--max-workers` alongside the existing documented flags.
- [x] 7.2 [cleanup] Run `PYTHONPATH=src pytest -q` and confirm it is green, including every new
