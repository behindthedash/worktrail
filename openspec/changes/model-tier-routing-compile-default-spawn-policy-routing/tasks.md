## 1. Routed default spawn in the conductor
...
## 2. Tests (tests/conductor/test_compile.py + tests/orchestrator)
...
- [ ] 3.1 Repo-wide grep confirms zero new `os.environ` reads in `src/worktrail/conductor/compile.py` and no new env-var names introduced anywhere.
- [ ] 3.2 `PYTHONPATH=src pytest -q` green, including the golden `worktrail.orchestrator.orchestrate check` replay (`PYTHONPATH=src python3 -m worktrail.orchestrator.orchestrate check`).
