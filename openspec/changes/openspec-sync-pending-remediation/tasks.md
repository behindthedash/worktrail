...
- [ ] 1.1 In `src/worktrail/router/dashboard.py`, implement
...
- [ ] 1.2 In `tests/router/test_dashboard.py`, cover unsynced and reconciled
      deltas, a change with no deltas, and verify-pending precedence.

## 2. Existing drain-row integration

- [ ] 2.1 In `tests/drain/test_drain.py`, cover the modified
...
- [ ] 3.1 [e2e] Run `PYTHONPATH=src pytest -q tests/router/test_dashboard.py
...
- [ ] 3.3 [cleanup] Run `openspec validate
