...
- [ ] 1.2 In the same file, add CLI-flag-shaped token recognition: a regex matching GNU
...
- [ ] 1.3 Update the module docstring and `check()`'s own docstring to describe the grace
...
- [ ] 1.4 Bump the `version` field in `pyproject.toml` and `.codex-plugin/plugin.json` to the
  same next patch version (this repo's `CI: Version Bump Check` fails any PR touching
  `src/worktrail/**` without one).

## 2. Tests

- [ ] 2.1 Extend `tests/router/test_check_brief_staleness.py` extraction coverage: a
...
- [ ] 2.2 Extend the history-search fixture coverage: a commit landing a few seconds *before*
...
- [ ] 2.3 Add PR-lookup coverage (mock or fixture-driven, following the existing `gh`-lookup
