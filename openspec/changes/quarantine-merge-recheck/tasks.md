...
- [ ] 1.1 In `src/worktrail/orchestrator/verify.py`, add
...
- [ ] 2.1 In `Verifier.verify_one`, in the existing `if not ok:` block, compute `violation` and
...
- [ ] 3.1 In `tests/orchestrator/test_verify.py`, add a unit test for
...
- [ ] 3.2 Add an integration-level test through `run_all`/`verify_one` reproducing the PR #339
...
- [ ] 3.3 Add a test confirming a confirmed self-merge violation (existing
      `test_resolve_worker_self_merge_surfaces_distinctly_not_quarantined` scenario shape) is
      unaffected by this change — the recheck must not run for it, and the violation still lands
      in `self_merged`, not `merged`.
- [ ] 3.4 [e2e] Run `PYTHONPATH=src pytest -q` and
