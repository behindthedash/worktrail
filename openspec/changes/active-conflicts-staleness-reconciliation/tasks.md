...
- [ ] 1.1 Add `_extract_path_candidate(entry: str) -> str` helper: return the
...
- [ ] 1.2 Add `_is_stale(record: Dict[str, Any], repo_dir: Path, base_branch:
...
- [ ] 1.3 Rewrite `_active_conflicts()` to return `{"live": [...], "stale":
...
- [ ] 1.4 Update `cmd_active_conflicts` to print the partitioned dict.
...
- [ ] 3.2 Re-read `#active-conflicts-scan`'s two existing citation sites
  (`#sibling-worktree-check`, `implement` pipeline step 1) to confirm no
  other text there assumes the old flat-array shape.

## 4. Tests

- [ ] 4.1 Unit tests for `_extract_path_candidate` covering a clean path, a
  path with trailing descriptive text, and an empty string.
- [ ] 4.2 Unit tests for `_is_stale` covering: worktree exists (live); no
  ...
  `files_changed` (live).
- [ ] 4.3 Unit tests for `_active_conflicts`'s partitioning against a
  temp run-record directory with a mix of live/stale/non-matching/terminal
  fixture records.
- [ ] 4.4 Unit tests for `cmd_reconcile`: stale-at-call-time closes the
  record with the expected `final_status`/`merge_result`; no-longer-stale
  leaves the record unmodified.
- [ ] 4.5 Update the existing `claim` conflict test(s) if any assert against
  the old flat-list shape from `_active_conflicts`.

## 5. Verification

- [ ] 5.1 [e2e] `PYTHONPATH=src pytest -q`
- [ ] 5.2 [e2e] `PYTHONPATH=src python3 -m worktrail.orchestrator.orchestrate check`
- [ ] 5.3 [e2e] `PYTHONPATH=src pytest -q tests/test_plugin_surface.py` to
