...
def view(
    ...
    return {
        ...
        "statusCheckRollup": rollup or GREEN,
        "headRefOid": head,
...
class FakeRun:
    ...
    def __init__(
        self, views, merges=None, remote_url="https://github.com/o/r.git", runs="[]"
    ):
...
class CleanGreenPath(unittest.TestCase):
    def test_merges_and_cleans_up(self):
        ...
        self.assertTrue(
            any(
                c[:5] == ["git", "-C", "/repo", "branch", "-D"]
                and c[-1] == "001-x/task-002"
                for c in run.calls
            )
        )
...
class RequiredCheckNotYetScheduledPath(unittest.TestCase):
    ...
    def test_empty_rollup_polls_again_instead_of_merging_immediately(self):
        run = FakeRun(
            {
                "run/feature-1": [
                    ...
                    view(
                        rollup=GREEN
                    ),  # second poll: the required "build" check landed
                ]
            }
        )
        spawn = FakeSpawn()
        ...
        self.assertGreaterEqual(len(run.find("gh", "pr", "view", "run/feature-1")), 2)
        ...
        run = FakeRun(
            {
                "run/feature-1": [
                    view(
                        mergeable="MERGEABLE", rollup=[]
                    ),  # ensure_mergeable: proceeds
                    _empty_rollup_view(
                        mergeable="CONFLICTING"
                    ),  # every poll from here on
                ]
            }
        )
        spawn = FakeSpawn()
        ...
        run = FakeRun(
            {
                "run/feature-1": [
                    view(
                        state="OPEN", auto_merge_request={"enabledBy": {"login": "bot"}}
                    ),
                    view(state="MERGED"),
                ]
            }
...
def _rt_thread(thread_id, path, resolved, created_at="2026-08-01T00:00:00Z"):
    return {
        ...
        "comments": {
            "nodes": [
                {
                    "author": {"login": "security-review-llm"},
                    "body": "possible finding",
...
class BaseFailureQuarantinesDependents(unittest.TestCase):
    def test_dependent_skipped_when_base_red(self):
        ...
        run = FakeRun(
            {"run/base": [view(rollup=RED)], "run/feature-1": [view()]},
            runs=json.dumps(
                [{"databaseId": 9, "conclusion": "FAILURE", "headSha": "abc"}]
            ),
        )
        ...
        v = mk(run, spawn, "/tmp/x")
        res = v.run_all(
            [base, feat], {"base": "run/base", "feature-1": "run/feature-1"}
        )
...
class NoAutomergeLabelGate(unittest.TestCase):
    ...
    def test_labeled_pr_blocks_the_branch_protection_fallback_path(self):
        ...
        run = FakeRun(
            ...
            merges={
                "run/feature-1": 1
            },  # direct merge blocked -> would try --auto fallback
        )
        spawn = FakeSpawn()
        v = mk(run, spawn, "/tmp/x")
        ok, msg = v.auto_merge(FEATURE, "run/feature-1")
...
class ClassifyChecks(unittest.TestCase):
    def test_shapes(self):
        ...
        self.assertEqual(
            verify.classify_checks([{"name": "t", "status": "IN_PROGRESS"}]), (True, [])
        )
        self.assertEqual(
    ...
    def test_required_check_missing_from_rollup_is_pending_not_green(self):
        ...
        self.assertEqual(
            verify.classify_checks(None, required=["Version bump check"]), (True, [])
        )
        self.assertEqual(
            verify.classify_checks([], required=["Version bump check"]), (True, [])
        )
    ...
    def test_required_check_present_and_green_stays_green(self):
        self.assertEqual(verify.classify_checks(GREEN, required=["build"]), (False, []))
...
class DeferToArmedAutoMerge(unittest.TestCase):
    ...
    def test_defers_instead_of_racing_when_armed(self):
        armed = view(
            state="OPEN", auto_merge_request={"enabledBy": {"login": "ci-bot"}}
        )
        ...
        self.assertEqual(msg, "")
        self.assertFalse(
            run.find("gh", "pr", "merge", "run/feature-1"),
...
class RequiredChecksPreflight(unittest.TestCase):
    ...
    def test_ineligible_branch_never_attempts_direct_merge(self):
        ...
        verify.automerge_preflight.required_checks_gate = lambda *args, **kwargs: (
            False,
            "base has zero required status checks",
        )
        try:
            ok, msg = v.auto_merge(FEATURE, "run/feature-1")
        finally:
    ...
    def test_preflight_query_error_falls_back_to_auto_merge(self):
        ...
        verify.automerge_preflight.required_checks_gate = lambda *args, **kwargs: (
            False,
            "could not query required status checks for o/r@dev (gh api failed after 3 attempts)",
        )
        try:
            ok, msg = v.auto_merge(FEATURE, "run/feature-1")
        finally:
        ...
        auto_merge_calls = [
            c for c in run.find("gh", "pr", "merge", "run/feature-1") if "--auto" in c
        ]
        self.assertTrue(
            auto_merge_calls, "expected a `gh pr merge --auto` fallback call"
        )
    ...
    def test_preflight_query_error_and_method_rejected_retries_other_methods(self):
        ...
        auto_squash_calls = [
            c for c in merge_calls if "--auto" in c and "--squash" in c
        ]
    ...
    def test_query_error_fallback_records_structured_event_on_failure(self):
        ...
        self.assertIn("feature-1", v._preflight_fallbacks)
        self.assertEqual(
            v._preflight_fallbacks["feature-1"]["outcome"], "fallback_failed"
        )
    ...
    def test_run_all_returns_preflight_fallbacks(self):
        ...
        self.assertEqual(
            result["preflight_fallbacks"]["feature-1"]["outcome"], "queued"
        )
...
class CleanupNoOps(unittest.TestCase):
    ...
    def test_cleanup_deletes_remote_group_branch(self):
        ...
        self.assertTrue(
            run.find(
                "git", "-C", "/repo", "push", "origin", "--delete", "run/feature-1"
            ),
...
class ReuseExistingVerifyWorktree(unittest.TestCase):
    def test_verify_worktree_reused_on_second_call(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            worktree_add_calls_after_first = [
                c
                for c in run.calls
            ...
            worktree_add_calls_after_second = [
                c
                for c in run.calls
                if c[:5] == ["git", "-C", "/repo", "worktree", "add"]
            ]
...
class AutoMergeEdgeCases(unittest.TestCase):
    def test_closed_pr_fails_merge(self):
        ...
        run = FakeRun({"run/feature-1": [closed_view]}, merges={"run/feature-1": 1})
        spawn = FakeSpawn()
        v = mk(run, spawn, "/tmp/x")
        ok, msg = v.auto_merge(FEATURE, "run/feature-1")
...
class ParallelVerifyWave(unittest.TestCase):
    ...
    def test_two_independent_groups_verified_in_one_wave(self):
        ...
        self.assertEqual(res["quarantined"], {})
        self.assertTrue(
            any("VERIFY WAVE [parallel x2]" in line for line in logs),
...
class CumulativePostMergeGate(unittest.TestCase):
    ...
    def test_no_post_merge_regressed_accumulator_falls_back_to_quarantined(self):
        ...
        merged, quarantined = [], {}
        v.verify_one(
            FEATURE, "run/feature-1", None, merged, quarantined, threading.Lock()
        )
    ...
    def test_regression_blocks_a_second_group_before_it_attempts_to_merge(self):
        ...
        self.assertFalse(
            run.find("gh", "pr", "view", "run/feature-2")
        )  # never even checked
    ...
    def test_cumulative_regression_shared_across_separate_verifier_instances(self):
        ...
        v1 = mk(
            ...
            merge_lock=shared_lock,
            cumulative_regression=shared_regression,
        )
...
class RetargetDependent(unittest.TestCase):
    ...
    def test_dependent_retargeted_base_is_not(self):
        ...
        self.assertIn(
            ["gh", "api", "repos/o/r/pulls/7", "-X", "PATCH", "-f", "base=dev"],
            run.calls,
        )
    ...
    def test_retarget_falls_back_to_pr_edit_when_pr_number_unresolvable(self):
        ...
        self.assertTrue(
            any("run/feature-1" in c and "--base" in c and "dev" in c for c in edits)
        )
...
class BranchProtectionAutoMergePath(unittest.TestCase):
    ...
    def _make_protection_run(self, auto_rc=0):
        ...
        def run(cmd):
            if cmd[:3] == ["gh", "pr", "merge"]:
                ...
                if "--auto" in cmd:
                    return Proc(
                        auto_rc, "", "" if auto_rc == 0 else "auto-merge not allowed"
                    )
                return Proc(1, "", "the base branch policy prohibits the merge")
            return base(cmd)
    ...
    def test_generic_merge_error_does_not_trigger_auto_fallback(self):
        ...
        auto_calls = [
            c for c in base.calls if c[:3] == ["gh", "pr", "merge"] and "--auto" in c
        ]
...
class VerifyOneMethod(unittest.TestCase):
    ...
    def test_queued_automerge_goes_to_armed_not_merged(self):
        ...
        v.verify_one(
            FEATURE, "run/feature-1", None, merged, quarantined, lock, armed=armed
        )
    ...
    def test_green_path_cleanup_only_delivered_tasks(self):
        ...
        v.verify_one(
            two_task_group, "run/feature-1", delivered, merged, quarantined, lock
        )

        self.assertEqual(merged, ["feature-1"])
        branch_dels = [
            c[-1] for c in run.calls if c[:5] == ["git", "-C", "/repo", "branch", "-D"]
        ]
        self.assertIn(
            "001-x/task-002", branch_dels, "delivered task branch must be deleted"
        )
        self.assertNotIn(
            "001-x/task-003", branch_dels, "non-delivered task branch must be kept"
        )
    ...
    def test_run_all_over_verify_one_preserves_wave_ordering(self):
        ...
        self.assertLess(base_idx, feat_idx, "base must be merged before its dependent")
    ...
    def test_dependent_retargeted_to_base_before_verify(self):
        ...
        self.assertEqual(merged, ["feature-1"])
        self.assertIn(
            ["gh", "api", "repos/o/r/pulls/1", "-X", "PATCH", "-f", "base=dev"],
            run.calls,
...
class SquashMergeDetection(unittest.TestCase):
    ...
    def test_merge_method_detected_once_and_cached(self):
        ...
        repo_view_calls = [c for c in run.calls if c[:3] == ["gh", "repo", "view"]]
        self.assertEqual(
            len(repo_view_calls),
            1,
...
class AutoMergeMethodFallback(unittest.TestCase):
        ...
        class MethodFallbackRun:
            ...
            def __call__(self, cmd):
                ...
                if cmd[:3] == ["gh", "pr", "merge"]:
                    ...
                    return Proc(
                        rc, "", "" if fallback_succeeds else "method also not allowed"
                    )
                base = FakeRun({"run/feature-1": [view()]})
                return base(cmd)
    ...
    def test_enablepullrequestautomerge_retries_squash(self):
        ...
        self.assertEqual(
            len(auto_squash_calls), 1, "Expected one --auto --squash retry"
        )
    ...
    def test_method_cached_after_successful_fallback(self):
        ...
        auto_squash_second = [
            ...
            if "--squash" in c and "feature-2" in " ".join(str(a) for a in c)
        ]
        self.assertTrue(
...
class WorkerScopeViolation(unittest.TestCase):
    ...
    def test_forbidden_paths_touched_helper_filters_deny_list_only(self):
        ...
        v = mk(run, FakeSpawn(), "/tmp/x")
        self.assertEqual(
            v._forbidden_paths_touched("presha", "gb"), [".github/workflows/x.yml"]
        )
    ...
    def test_an_openspec_run_guards_openspec_not_docs_specs(self):
        ...
        self.assertEqual(
            v._forbidden_paths_touched("presha", "gb"),
            ["openspec/changes/080-x/tasks.md"],
        )
    ...
    def test_touched_not_declared_is_logged_not_gated(self):
        ...
        self.assertTrue(
            any("plan-audit" in m and "src/surprise.py" in m for m in logged), logged
        )
...
class WorkerSelfMergeViolation(unittest.TestCase):
    ...
    def test_detect_self_merge_none_pre_status_fails_open(self):
        ...
        v = mk(run, FakeSpawn(), "/tmp/x")
        self.assertFalse(
            v._detect_self_merge("resolve", FEATURE, "run/feature-1", None)
        )
