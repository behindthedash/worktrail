...
class WriteGroupJournalTests(unittest.TestCase):
    def test_writes_new_journal(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            integrate._write_group_journal(path, "base", "https://github.com/o/r/pull/1",
                                           "run-001/base", "OPEN")
            data = json.loads(Path(path).read_text())
            self.assertEqual(data["groups"]["base"]["pr_url"], "https://github.com/o/r/pull/1")
            self.assertEqual(data["groups"]["base"]["state"], "OPEN")

    def test_updates_existing_journal(self):
        with tempfile.TemporaryDirectory() as t:
            path = os.path.join(t, "journal.json")
            Path(path).write_text(json.dumps({"groups": {"base": {"state": "OPEN", "pr_url": "", "head_branch": ""}}}))
            integrate._write_group_journal(path, "base", "https://github.com/o/r/pull/1",
                                           "run-001/base", "MERGED")
            data = json.loads(Path(path).read_text())
    ...
    def test_exception_is_printed_not_raised(self):
        with patch.object(progress, "atomic_write_text", side_effect=OSError("disk full")):
            with tempfile.TemporaryDirectory() as t:
                path = os.path.join(t, "journal.json")
...
class MarkIntegrateCompleteTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            Path(path).write_text(json.dumps({
                "groups": {
                    ...
                    "feature-1": {"state": "OPEN", "pr_url": "", "head_branch": ""},
                }
            }))
    ...
    def test_no_mark_when_group_missing(self):
        with tempfile.TemporaryDirectory() as t:
            path = os.path.join(t, "journal.json")
            Path(path).write_text(json.dumps({
                "groups": {"base": {"state": "MERGED", "pr_url": "", "head_branch": ""}}
            }))
            complete = integrate._mark_integrate_complete_if_terminal(
                path, self._groups(["base", "feature-1"])  # feature-1 missing
            )
            self.assertFalse(complete)

    def test_none_journal_path_returns_false(self):
        result = integrate._mark_integrate_complete_if_terminal(None, self._groups(["base"]))
...
class SyntheticFanoutTests(unittest.TestCase):
    ...
    def test_creates_branch_per_task(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            result = subprocess.run(["git", "-C", str(repo), "branch"],
...
class IntegrateOneDepOnQuarantinedTests(unittest.TestCase):
    def test_dep_on_quarantined_cascades(self):
        g = {"name": "feature-1", "tasks": ["TASK-002"], "reqs": [], "depends_on": ["base"]}
        quarantined = {"base": "some reason"}
        group_branch = {}
        status = {"TASK-002": "done"}
        tasks = [{"id": "TASK-002", "deps": ["TASK-001"], "files": ["src/b.ts"],
                  "kind": "impl", "status": "done"}]

        result = integrate.integrate_one(
            g=g, repo=Path("/fake/repo"), spec_id="001-spec", tasks=tasks,
            remote="origin", run_id="run-001", base="main",
            journal_path=None, status=status, group_branch=group_branch,
...
class AutoResolveAddAddInitTests(unittest.TestCase):
    ...
    def _add_init_branch(self, repo: Path, branch: str, content: str) -> None:
        ...
        subprocess.run(["git", "-C", str(repo), "checkout", "-q", "-B", branch, "main"],
                       check=True, capture_output=True)
        d = repo / "tools"
        d.mkdir(exist_ok=True)
        (d / "__init__.py").write_text(content)
        subprocess.run(["git", "-C", str(repo), "add", "-A"], check=True, capture_output=True)
        subprocess.run(["git", "-C", str(repo), "commit", "-q", "-m", f"add init via {branch}"],
                       check=True, capture_output=True)
    ...
    def _conflict_merge(self, repo: Path, into: str, other: str) -> None:
        ...
        subprocess.run(["git", "-C", str(repo), "checkout", "-q", into],
                       check=True, capture_output=True)
        m = subprocess.run(["git", "-C", str(repo), "merge", "--no-edit", other],
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            def add_branch(branch, content):
                ...
                subprocess.run(["git", "-C", str(repo), "add", "-A"],
                               check=True, capture_output=True)
                subprocess.run(["git", "-C", str(repo), "commit", "-q", "-m", branch],
                               check=True, capture_output=True)
    ...
    def test_modify_modify_init_not_resolved(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            subprocess.run(["git", "-C", str(repo), "add", "-A"], check=True, capture_output=True)
            subprocess.run(["git", "-C", str(repo), "commit", "-q", "-m", "base init"],
                           check=True, capture_output=True)
