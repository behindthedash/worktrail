...
class SplitClaudeMdTests(unittest.TestCase):
    def test_moves_content_and_stamps_import_line(self):
        ...
        self.assertEqual(
            (repo / "AGENTS.md").read_text(), "# my repo\nsome real content\n"
        )
    ...
    def test_existing_agents_md_with_unsplit_claude_md_warns_and_skips(self):
        ...
        self.assertEqual(
            (repo / "AGENTS.md").read_text(), "separate pre-existing content\n"
        )
...
class BuildRulesetsDriftGuardWorkflowTests(unittest.TestCase):
    ...
    def test_has_app_token_step_and_no_secrets_github_token_for_rulesets_api(self):
        ...
        rulesets_steps = [
            s for s in steps if "run" in s and "rulesets_sync.py" in s["run"]
        ]
    ...
    def test_credential_guard_if_conditions_present(self):
        ...
        check_step = next(
            s
            for s in steps
            if s.get("name") == "Check committed rulesets against live GitHub rulesets"
        )
        self.assertEqual(
            check_step["if"],
        ...
        apply_step = next(
            s
            for s in steps
            if s.get("name") == "Apply committed rulesets to live GitHub rulesets"
        )
        self.assertEqual(
            apply_step["if"],
            "${{ github.event_name == 'push' && steps.app-token.outputs.token != '' }}",
...
class ComputeDriftTests(unittest.TestCase):
    ...
    def test_ruleset_structural_drift_detected_but_extra_required_check_is_not(self):
        ...
        self.assertIn(
            "required_status_checks is intentionally excluded", drift[0]["detail"]
        )
...
class ProposeTests(unittest.TestCase):
    ...
    def test_fresh_repo_writes_everything(self):
        ...
        self.assertTrue(
            (repo / ".github" / "workflows" / "worktrail-auto-merge.yml").is_file()
        )
    ...
    def test_propose_writes_routing_yaml_when_absent(self):
        ...
        with mock.patch.dict(
            "os.environ", {"WORKTRAIL_ROUTING_FILE": str(routing_path)}
        ):
            rc, result = self._run_propose(repo)
        self.assertEqual(rc, 0)
    ...
    def test_propose_leaves_existing_routing_yaml_alone(self):
        ...
        with mock.patch.dict(
            ...
            rc, result = self._run_propose(repo)
        self.assertEqual(rc, 0)
        self.assertFalse(any(w.startswith("routing.yaml") for w in result["written"]))
        self.assertTrue(
            any(
                "routing.yaml" in s and "already exists" in s for s in result["skipped"]
            )
        )
        self.assertEqual(
    ...
    def test_fresh_repo_writes_openspec_validate_workflow_and_gates_rulesets(self):
        ...
        self.assertEqual(
            workflow_path.read_text(), repo_init.build_openspec_validate_workflow()
        )
    ...
    def test_rerun_after_workflow_written_is_noop_on_workflow_and_required_checks(self):
        ...
        self.assertEqual(rc, 0)
        self.assertNotIn(
            repo_init.OPENSPEC_VALIDATE_WORKFLOW_RELPATH, result["written"]
        )
        self.assertTrue(
            any(
                repo_init.OPENSPEC_VALIDATE_WORKFLOW_RELPATH in s
    ...
    def test_hand_authored_workflow_and_ruleset_missing_check_is_full_noop(self):
        ...
        self.assertEqual(
            (rulesets_dir / "protect-dev.json").read_text(), dev_ruleset_before
        )
        self.assertEqual(
    ...
    def test_already_onboarded_repo_patches_existing_rulesets_without_altering_other_rules(
        ...
        self.assertIn(repo_init.OPENSPEC_VALIDATE_WORKFLOW_RELPATH, result["written"])
        self.assertTrue((repo / repo_init.OPENSPEC_VALIDATE_WORKFLOW_RELPATH).is_file())
        ...
        dev_rsc = next(
            r
            for r in json.loads((rulesets_dir / "protect-dev.json").read_text())[
                "rules"
            ]
        ...
        prd_rsc = next(
            r
            for r in json.loads((rulesets_dir / "protect-prd.json").read_text())[
                "rules"
            ]
            if r["type"] == "required_status_checks"
        )
    ...
    def test_fresh_repo_writes_rulesets_drift_guard_workflow_and_script(self):
        ...
        self.assertTrue(
            (repo / "scripts" / "ci" / "rulesets" / "rulesets_sync.py").is_file()
        )
        self.assertTrue(
    ...
    def test_fresh_repo_writes_dependabot_manifest_check_files(self):
        ...
        self.assertIn(
            repo_init.DEPENDABOT_CHECK_REQUIREMENTS_RELPATH, result["written"]
        )
        ...
        self.assertEqual(
            script_path.read_text(), repo_init.DEPENDABOT_MANIFEST_CHECK_PY
        )
        self.assertEqual(
            requirements_path.read_text(),
            repo_init.DEPENDABOT_MANIFEST_CHECK_REQUIREMENTS_TXT,
        )
    ...
    def test_rerun_leaves_hand_customized_dependabot_workflow_untouched(self):
        ...
        self.assertTrue(
            any(
                repo_init.DEPENDABOT_CHECK_WORKFLOW_RELPATH in s
                for s in result["skipped"]
            )
        )
        self.assertNotIn(repo_init.DEPENDABOT_CHECK_WORKFLOW_RELPATH, result["written"])

    def test_existing_dependabot_script_with_missing_workflow_writes_only_workflow(
        self,
    ):
        repo = _tmp_repo()
        script_path = repo / repo_init.DEPENDABOT_CHECK_SCRIPT_RELPATH
        ...
        self.assertTrue(
            any(
                repo_init.DEPENDABOT_CHECK_SCRIPT_RELPATH in s
                for s in result["skipped"]
            )
        )
        self.assertTrue(
            any(
                repo_init.DEPENDABOT_CHECK_REQUIREMENTS_RELPATH in s
                for s in result["skipped"]
            )
        )
        ...
        self.assertTrue((repo / repo_init.DEPENDABOT_CHECK_WORKFLOW_RELPATH).is_file())
    ...
    def test_check_mode_reports_dependabot_manifest_check_state_without_writing(self):
        ...
        self.assertFalse(
            (repo / repo_init.DEPENDABOT_CHECK_REQUIREMENTS_RELPATH).exists()
        )
    ...
    def test_dependabot_check_job_name_never_added_to_required_status_checks(self):
        ...
        dev_contexts = [
            c["context"] for c in dev_rsc["parameters"]["required_status_checks"]
        ]
        ...
        prd_contexts = [
            c["context"] for c in prd_rsc["parameters"]["required_status_checks"]
        ]
        self.assertNotIn(repo_init.DEPENDABOT_CHECK_JOB_NAME, prd_contexts)

    def test_nonexistent_repo_errors(self):
        args = mock.Mock(
            repo="/nonexistent/path/xyz", branch_model="2", check=False, as_json=True
        )
        rc = repo_init.cmd_propose(args)
    ...
    def test_with_aspens_writes_add_ons_block_and_runs_configure(self):
        ...
        with mock.patch.object(
            repo_init, "enable_aspens", return_value=(True, None)
        ) as ea:
    ...
    def test_aspens_warning_surfaces_without_failing_propose(self):
        ...
        with mock.patch.object(
            repo_init,
            "enable_aspens",
            return_value=(False, "aspens doc init did not produce ..."),
        ):
            rc, result = self._run_propose(repo, with_aspens=True)
        self.assertEqual(rc, 0)
    ...
    def test_with_gitnexus_runs_indexing_and_leaves_policy_bare(self):
        repo = _tmp_repo()
        with mock.patch.object(
            repo_init, "enable_gitnexus", return_value=(True, None)
        ) as eg:
    ...
    def test_gitnexus_warning_surfaces_without_failing_propose(self):
        ...
        with mock.patch.object(
            repo_init,
            "enable_gitnexus",
            return_value=(False, "gitnexus analyze did not produce ..."),
        ):
            rc, result = self._run_propose(repo, with_gitnexus=True)
        self.assertEqual(rc, 0)
    ...
    def test_with_gitnexus_already_indexed_wires_through_skipped(self):
        repo = _tmp_repo()
        with mock.patch.object(
            repo_init, "enable_gitnexus", return_value=(False, None)
        ) as eg:
            rc, result = self._run_propose(repo, with_gitnexus=True)
        self.assertEqual(rc, 0)
        eg.assert_called_once()
    ...
    def test_stale_automerge_workflow_surfaces_in_drift_and_is_not_rewritten(self):
        ...
        self.assertEqual(
            automerge_path.read_text(), "name: hand-edited stale content\n"
        )
        self.assertIn(
            f"{repo_init.AUTOMERGE_WORKFLOW_RELPATH} (already exists)",
            result["skipped"],
        )
...
class EnableGitnexusTests(unittest.TestCase):
    ...
    def test_successful_indexing_reports_configured(self):
        ...
        with mock.patch(
            "worktrail.onboarding.repo_init._run", side_effect=fake_run
        ) as run:
            configured, warning = repo_init.enable_gitnexus(repo)
        self.assertTrue(configured)
        self.assertIsNone(warning)
...
class ApplyTests(unittest.TestCase):
    def _repo_with_rulesets(self, branches=("dev", "prd")) -> Path:
        ...
        for branch in branches:
            ruleset = repo_init.build_ruleset_for_branch(
                branch, "3" if "stg" in branches else "2"
            )
    ...
    def test_full_two_branch_apply_sequence(self):
        ...
        def fake_run(cmd, **kw):
            ...
            if joined == "gh api repos/acme/widget -q .default_branch":
                return subprocess.CompletedProcess(
                    cmd, 0, stdout=f"{state['default_branch']}\n", stderr=""
                )
            if joined == "gh api repos/acme/widget -q .delete_branch_on_merge":
                return subprocess.CompletedProcess(
                    cmd,
                    0,
                    stdout=f"{str(state['delete_branch_on_merge']).lower()}\n",
                    stderr="",
                )
            ...
            if joined == "gh api repos/acme/widget/branches/dev":
                ...
                return subprocess.CompletedProcess(
                    cmd, rc, stdout="", stderr="" if rc == 0 else "404"
                )
            ...
            if (
                joined
                == "gh api --method POST repos/acme/widget/branches/master/rename -f new_name=prd"
            ):
            ...
            if (
                joined
                == "gh api --method PATCH repos/acme/widget -f default_branch=dev"
            ):
                state["default_branch"] = "dev"
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if (
                joined
                == "gh api --method PATCH repos/acme/widget -f delete_branch_on_merge=true"
            ):
                state["delete_branch_on_merge"] = True
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if joined == "gh api repos/acme/widget/rulesets":
                return subprocess.CompletedProcess(
                    cmd, 0, stdout=json.dumps(state["rulesets"]), stderr=""
                )
            if cmd[:4] == ["gh", "api", "--method", "POST"] and joined.endswith(
                "rulesets --input -"
            ):
                payload = json.loads(kw["input"])
                state["rulesets"].append(
                    {"id": len(state["rulesets"]) + 1, "name": payload["name"]}
                )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        ...
        self.assertEqual(
            result["rulesets"]["protect-dev.json"], "created and verified live"
        )
        self.assertEqual(
            result["rulesets"]["protect-prd.json"], "created and verified live"
        )
        ...
        def fake_run(cmd, **kw):
            ...
            if " ".join(cmd) == "gh api repos/acme/widget/rulesets":
                return subprocess.CompletedProcess(
                    ...
                    stdout=json.dumps(
                        [
                            {"id": 1, "name": "protect-dev"},
                            {"id": 2, "name": "protect-prd"},
                        ]
                    ),
                    stderr="",
                )
        ...
        def fake_run(cmd, **kw):
            ...
            if joined == "gh variable list --json name -R acme/widget":
                return subprocess.CompletedProcess(
                    cmd,
                    0,
                    stdout=json.dumps([{"name": "RELEASE_NOTES_APP_ID"}]),
                    stderr="",
                )
            if joined == "gh secret list --json name -R acme/widget":
                return subprocess.CompletedProcess(
                    cmd,
                    0,
                    stdout=json.dumps([{"name": "RELEASE_NOTES_APP_PRIVATE_KEY"}]),
                    stderr="",
                )
    ...
    def test_reminder_printed_when_app_credentials_missing(self):
        ...
        self.assertTrue(
            any(
                "RELEASE_NOTES_APP_ID" in w and "RELEASE_NOTES_APP_PRIVATE_KEY" in w
...
class ResolveRepoDisplayNameTests(unittest.TestCase):
    ...
    def test_worktree_directory_name_does_not_leak_into_repo_name(self):
        ...
        with mock.patch.object(
            repo_init, "resolve_gh_repo", return_value="behindthedash/hearsay"
        ):
            self.assertEqual(
                repo_init.resolve_repo_display_name(worktree_path), "hearsay"
            )
...
class ApplyRulesetTests(unittest.TestCase):
    def test_creates_when_no_live_ruleset_shares_name(self):
        ...
        def fake_run(cmd, **kw):
            ...
            if joined == "gh api repos/acme/widget/rulesets":
                already_created = any(
                    c[:4] == ["gh", "api", "--method", "POST"] for c in calls
                )
        ...
        def fake_run(cmd, **kw):
            ...
            if joined == "gh api repos/acme/widget/rulesets":
                return subprocess.CompletedProcess(
                    cmd,
                    0,
                    stdout=json.dumps([{"id": 9, "name": "protect-dev"}]),
                    stderr="",
                )
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
