...
`version_bump_check.yml` already guards against for its own labeled/unlabeled
trigger (`tests/test_version_bump_workflow.py`), with an even sharper edge
...
def test_concurrency_group_keys_on_pr_action_and_label():
    ...
    stuck-check incident `version_bump_check.yml`'s own group already guards
