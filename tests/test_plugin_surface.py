...
def test_compile_gate_documents_failure_recovery():
    ...
    assert subagent.count("{#compile-gate}") == 1, (
        "`{#compile-gate}` is defined more than once in subagent-prompts.md -- "
        "the anchor must live in `#orchestrator-gates` and nowhere else"
    )
    ...
    # Each entry is a conjunction of alias groups: every group must match, but
    # a group's spellings alternate ("plan shape" / "plan-shape").
    missing = [
        name
        for name, groups in classes.items()
        if not all(any(n in gate for n in group) for group in groups)
    ]
    assert not missing, (
    ...
    offenders = [
        ...
        if p.is_file()
        and "inspect the error above before retrying" in p.read_text(errors="ignore")
    ]
    assert not offenders, (
