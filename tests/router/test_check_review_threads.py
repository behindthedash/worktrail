...
def _thread(
    ...
    author: str = "security-review-llm",
    body: str = "possible SQL injection",
) -> dict[str, Any]:
    return {
        "id": thread_id,
        "isResolved": resolved,
        ...
        "comments": {
            "nodes": [
                {"author": {"login": author}, "body": body, "createdAt": created_at}
            ]
        },
    }
...
class TestFetchReviewThreads(unittest.TestCase):
    def test_single_page(self) -> None:
        ...
        threads, warning = crt.fetch_review_threads(
            "acme", "widgets", 42, runner=runner
        )
        ...
        def runner(cmd: list[str], **kwargs: Any) -> _FakeResult:
            ...
            if calls["n"] == 1:
                return _threads_response(
                    [_thread("T_1", "a.py", False)], has_next=True, end_cursor="c1"
                )
    ...
    def test_gh_nonzero_exit_returns_none_with_warning(self) -> None:
        runner = _FakeRunner(
            {
                "gh api graphql": _FakeResult(
                    1, "", "GraphQL: Could not resolve to a PullRequest"
                ),
            }
        )
        threads, warning = crt.fetch_review_threads(
            "acme", "widgets", 999, runner=runner
        )
        self.assertIsNone(threads)
    ...
    def test_unexpected_shape_returns_none_with_warning(self) -> None:
        runner = _FakeRunner(
            {"gh api graphql": _FakeResult(0, json.dumps({"data": {}}))}
        )
        threads, warning = crt.fetch_review_threads(
            "acme", "widgets", 42, runner=runner
        )
...
class TestCorrelate(unittest.TestCase):
    ...
    def test_commit_touching_path_since_marks_addressed(self) -> None:
        threads = [
            _thread("T_1", "a.py", resolved=False, created_at="2026-08-01T00:00:00Z")
        ]
        runner = _FakeRunner(
            {
    ...
    def test_thread_with_no_path_or_comments_is_unaddressed_without_decision(
        self,
    ) -> None:
        thread = {
            "id": "T_1",
    ...
    def test_base_ref_commit_marks_addressed_when_head_has_no_match(self) -> None:
        ...
        grouped = crt.correlate(
            Path("/repo"), threads, [], runner=runner, base_ref="origin/main"
        )
        self.assertEqual([t["id"] for t in grouped["addressed"]], ["T_1"])
        self.assertEqual(grouped["unaddressed"], [])
    ...
    def test_base_ref_not_checked_when_head_already_matches(self) -> None:
        ...
        def runner(cmd: list[str], **kwargs: Any) -> _FakeResult:
            ...
            raise AssertionError(
                f"base_ref should not be checked when HEAD already matched: {cmd}"
            )
    ...
    def test_base_ref_not_checked_when_decision_already_matches(self) -> None:
        ...
        def runner(cmd: list[str], **kwargs: Any) -> _FakeResult:
            if cmd[:5] == ["git", "-C", "/repo", "log", "HEAD"]:
                return _FakeResult(0, "")
            raise AssertionError(
                f"base_ref should not be checked when decision already matched: {cmd}"
            )

        grouped = crt.correlate(
            Path("/repo"), threads, decisions, runner=runner, base_ref="origin/main"
        )
        self.assertEqual([t["id"] for t in grouped["addressed"]], ["T_1"])
...
class TestReplyBodyFor(unittest.TestCase):
    def test_decision_matched_thread_quotes_decision_text(self) -> None:
        thread = {
            "id": "T_1",
            "_matched_decision": "investigated, false positive, not fixing",
        }
...
class TestReplyAndResolve(unittest.TestCase):
    ...
    def test_reply_failure_short_circuits_before_resolve(self) -> None:
        runner = _FakeRunner(
            {"gh api graphql": _FakeResult(1, "", "permission denied")}
        )
        ok, err = crt.reply_and_resolve("T_1", "done", runner=runner)
        self.assertFalse(ok)
...
class TestCheck(unittest.TestCase):
        ...
        runner = _FakeRunner(
            {
                ...
                "gh api graphql": _threads_response(
                    [_thread("T_1", "a.py", resolved=True)]
                ),
            }
        )
        res = crt.check(Path("/repo"), 42, runner=runner)
    ...
    def test_addressed_thread_is_resolved_and_not_blocking(self) -> None:
        def runner(cmd: list[str], **kwargs: Any) -> _FakeResult:
            ...
            if (
                cmd[:2] == ["gh", "api"]
                and "query" in " ".join(cmd)
                and "reviewThreads" in " ".join(cmd)
            ):
    ...
    def test_unaddressed_thread_blocks_and_applies_no_automerge_label(self) -> None:
        ...
        self.assertFalse(
            any(
                "addPullRequestReviewThreadReply" in " ".join(c)
                or "resolveReviewThread" in " ".join(c)
                for c in calls
            )
        )
    ...
    def test_decision_log_addressed_thread_reply_quotes_decision_text(self) -> None:
        ...
        try:
            res = crt.check(
                Path("/repo"), 42, run_record_path=run_record_path, runner=runner
            )
        finally:
        ...
        self.assertTrue(res["resolved_now"][0]["resolved"])
        reply_calls = [
            c for c in calls if "addPullRequestReviewThreadReply" in " ".join(c)
        ]
