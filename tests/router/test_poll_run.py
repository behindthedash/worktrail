...
from worktrail.router import poll_run
...
class TestPollRun(unittest.TestCase):
    ...
    def test_exit_0_on_finish_with_pr(self):
        ...
        out = StringIO()
        with (
            patch.object(poll_run, "ensure_pr_risk_label", return_value=None),
    ...
    def test_exit_0_on_finish_with_pr_applies_label_correction(self):
        ...
        with (
            patch.object(
                ...
                lambda repo, pr, risk: (
                    seen.append((repo, pr, risk)) or "go:risk-medium"
                ),
            ),
            patch("sys.stdout", out),
        ):
        ...
        self.assertEqual(rc, 0)
        self.assertEqual(
            seen, [("/tmp/test-repo", "https://github.com/test/pr/42", "medium")]
        )
        output = out.getvalue()
    ...
    def test_exit_0_on_finish_without_pr_skips_label_correction(self):
        ...
        path = self._write_record(
            "finished.yaml", final_status="investigation_complete"
        )
        ...
        out = StringIO()
        with (
            patch.object(poll_run, "ensure_pr_risk_label", unexpected),
            patch("sys.stdout", out),
        ):
            rc = main(["--run", path, "--interval", "0", "--max-iterations", "1"])
        self.assertEqual(rc, 0)
    ...
    def test_exit_1_on_ceiling_reached(self):
        ...
        out = StringIO()
        with patch("sys.stdout", out):
            rc = main(["--run", path, "--interval", "0", "--max-iterations", "2"])
    ...
    def test_bounded_loop_enforced(self):
        ...
        with patch("time.sleep", mock_sleep):
            out = StringIO()
            with patch("sys.stdout", out):
                rc = main(["--run", path, "--interval", "1", "--max-iterations", "3"])
...
class PendingDecisionSurfacingTests(unittest.TestCase):
    ...
    def _write_record(self, filename: str, final_status=None, decisions=()) -> str:
        path = Path(self.tmp) / filename
        lines = [
            "run_id: test-run",
            "status: executing",
    ...
    def test_unresolved_ids_track_the_latest_event_per_decision(self):
        ...
        self.assertEqual(unresolved_decision_ids(record), ["dec-one", "dec-two"])
