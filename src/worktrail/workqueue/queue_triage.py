...
def evaluate_group(
    ...
    *,
    ...
    "candidates_by_brief", "premise_by_brief"}]` -- rather than the raw string
    directly, so a caller fanning out across multiple groups can `.extend()`
    every call's result into one flat list, without a shape special-case for
    the archived short-circuit. `raw_text` is untouched worker output (or, in
    the archived case, the synthesized equivalent); parsing/validation is
    `parse_verdicts`'s job, not this function's. `candidates_by_brief` maps
    each brief id to the list of candidate change ids 2.1's
    `rank_change_candidates()` presented to the evaluator for it (`[]` for the
    archived short-circuit and for `{no_repo_key}` groups, which have no
    target repo to rank against) -- a caller passes this straight through to
    `parse_verdicts()` so a `fold-into-change` naming anything else is
    rejected. `premise_by_brief` (4.1(d)) maps each brief id to its
    mechanical premise-check results (`premise_check.run_premise_check()`,
    run against `cwd` only when `repo` is not `{no_repo_key}` -- a repo-less
    group has no checkout to check claims against), embedded in the prompt
    text under each brief's own line (`format_premise_block()`) and passed
    through unmodified for `parse_verdicts()` to copy onto each `Verdict`.
    """
...
def evaluate_briefs(
    ...
    *,
    ...
    verdicts: list[Verdict] = []
    for result in evaluate_group(repo, briefs, agent=agent, cwd=cwd):
...
def main(argv: list[str] | None = None) -> int:
    ...
    evaluate_parser.add_argument(
        ...
        "decision consumption, and the escalation matrix (default ~/projects)",
    )
