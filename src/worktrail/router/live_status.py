...
def _read_ppid(pid_dir: Path) -> Optional[int]:
    try:
...
def list_procs(proc_root: Path = Path("/proc")) -> List[Dict[str, Any]]:
    ...
    for entry in proc_root.iterdir():
        ...
        try:
            ...
            continue
        procs.append(
            {"pid": int(entry.name), "ppid": _read_ppid(entry), "argv": argv, "start_time": start_time}
        )
    return procs
...
def compute(repos_dir: Path, proc_root: Path, runlock_held) -> Dict[str, Any]:
    ...
    orphaned = [l for l in locks if not any(l["repo"] in rp[0] for rp in run_lock_paths)]
...
def render(result: Dict[str, Any]) -> str:
    ...
    for o in result["orphaned_locks"]:
        lines.append(f"  ⚠️  {Path(o['repo']).name} — held lock with no matching process: {o['lock']}")
    return "\n".join(lines)


def main(argv=None) -> int:
    p = argparse.ArgumentParser(description="Cross-repo live-activity view")
    p.add_argument("--repos", default="~/projects", help="parent dir containing repo checkouts")
    p.add_argument("--json", action="store_true")
    args = p.parse_args(argv)

    from . import dashboard  # local import: avoids a module-load cost when unused

    result = compute(Path(args.repos).expanduser(), Path("/proc"), dashboard._runlock_held)
    if args.json:
