"""Vendored copy of worktrail's own `scripts/ci/check_gitleaks_signal_integrity.py`.

`gitleaks detect` exits 0 and logs "no leaks found" both when a scan genuinely
covered the range cleanly and when the range covered zero commits, so a
required secrets check can pass without having looked at anything. This script
is what tells those apart; `worktrail-repo-init` scaffolds it into a target
repo verbatim -- as a string constant here rather than a packaged data file,
matching `rulesets_drift_guard_template.py` and
`dependabot_manifest_check_template.py`, so the CLI has no runtime dependency
on package-data resolution.

Source of truth: this repo's `scripts/ci/check_gitleaks_signal_integrity.py`
(itself the datalena reference copy). Update this constant by hand if that
script changes -- there is no automated sync, and
`test_vendored_gitleaks_script_matches_the_source_of_truth` fails when the two
drift apart. The constant is a RAW string so the script's own regexes survive
verbatim.
"""

from __future__ import annotations

CHECK_GITLEAKS_SIGNAL_INTEGRITY_PY = r'''#!/usr/bin/env python3
